if not exists (select * from sysobjects where name='JewelryType')
    create table JewelryType (Id int identity (1,1), Name nvarchar (100),PricePerG Float (5), PRIMARY KEY (Id))

if not exists (select * from sysobjects where name='Jewelry')
    create table Jewelry (Id int identity (1,1),ShopId nvarchar(100),Weight Float (5), TypeId int, Quantity int, DateAdded DATETIME, PhotoFilename nvarchar(500), IsDeleted bit, PRIMARY KEY (Id),
